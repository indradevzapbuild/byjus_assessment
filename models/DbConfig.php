...
class DbConfig
{
    ...
    static function getConf(){
        ...
        if (($url=getenv("DATABASE_URL"))!==false) {
            ...
            self::$_port = $url["port"];
        } else if (file_exists($_SERVER['DOCUMENT_ROOT'] . $_SERVER['APP_ROOT'] . 'config/dbconfig.php')){
        ...
        } else if (file_exists($_SERVER['DOCUMENT_ROOT'] . $_SERVER['APP_ROOT'] . 'config/.dbconfig.json')){
            // json config
            $dbConfig = json_decode(file_get_contents($_SERVER['DOCUMENT_ROOT'].$_SERVER['APP_ROOT'].'config/.dbconfig.json'), true);
            self::$_username = $dbConfig['user'];
            self::$_password = $dbConfig['pass'];
            self::$_database = $dbConfig["database"];
            self::$_hostname = $dbConfig['host'];
